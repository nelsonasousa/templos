Public Class Frm_Localiza_Pais
    Dim tipoCampo As System.Type
    Dim strIDPai As String
    Dim strNomePai As String

    Private Sub Frm_Localiza_Pais_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Leave
        If Me.Tag = "VwLocalizaPai" Then
            Frm_Membros.txtIDPai.Text = strIDPai
            Frm_Membros.txtNomePai.Text = strNomePai
        Else
            Frm_Membros.txtIDMae.Text = strIDPai
            Frm_Membros.txtNomeMae.Text = strNomePai
        End If
    End Sub

    Private Sub Frm_Localiza_Pais_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Me.Tag = "VwLocalizaPai" Then
            str_SQL = "SELECT Tbl_Pessoas.IDPessoa,Tbl_Pessoas.Pessoa FROM Tbl_Pessoas WHERE(((Tbl_Pessoas.Sexo) = 'M')) ORDER BY Tbl_Pessoas.Pessoa;"

        Else
            str_SQL = "SELECT Tbl_Pessoas.IDPessoa,Tbl_Pessoas.Pessoa FROM Tbl_Pessoas WHERE(((Tbl_Pessoas.Sexo) = 'F')) ORDER BY Tbl_Pessoas.Pessoa;"
        End If

        ds = Busca.RetornarDataSet(str_SQL)
    ...
    Private Sub cmbCampo_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbCampo.SelectedIndexChanged
        ...
        If tipoCampo.ToString = "System.Int64" Then
    ...
    Private Sub Dgv1_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Dgv1.RowEnter
        If Not Dgv1.Rows(e.RowIndex).Cells(0).Value Is DBNull.Value Then
            strIDPai = Dgv1.Rows(e.RowIndex).Cells(0).Value
            strNomePai = Dgv1.Rows(e.RowIndex).Cells(1).Value
        End If
    End Sub
